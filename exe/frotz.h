...
extern uint8_t *story_data;
extern uint32_t story_data_size;
extern uint32_t story_pos;
...
void 	os_finish_with_sample (zword);
...
bool 	os_process_arguments (int, char *[]);
int	    os_random_seed (void);
int  	os_read_file_name (char *, const char *, int);	
int     os_read_savefile(std::vector<uint8_t>& svf);
...
void 	os_stop_sample (zword);
int  	os_string_width (const zchar *);
void	os_init_setup (void);
void 	os_warn (const char *, ...);
void    os_write_savefile(std::vector<uint8_t> &vecBytes);
void	os_quit (void);
