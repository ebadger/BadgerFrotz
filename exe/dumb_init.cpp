...
static int user_screen_height = 28;
...
void os_init_startup_variables(const char * story)
{
	 char *p = NULL;
    /* Create nice default file names */

    f_setup.story_file = my_strdup((char *)story);
    //if (zoptind < argc)
    ...
    *p = '\0';	/* extension removed */
	...
	{
      f_setup.save_name = (char *)malloc(strlen(f_setup.story_name) * sizeof(char) + 5);
      strcpy(f_setup.save_name, f_setup.story_name);
      strcat(f_setup.save_name, EXT_SAVE);
    } 
	else 
	{ /* Set our auto load save as the name save */
      f_setup.save_name = (char *)malloc(strlen(f_setup.tmp_save_name) * sizeof(char) + 5);
      strcpy(f_setup.save_name, f_setup.tmp_save_name);
      free(f_setup.tmp_save_name);
    }

    f_setup.script_name = (char *)malloc(strlen(f_setup.story_name) * sizeof(char) + 5);
    strcpy(f_setup.script_name, f_setup.story_name);
    strcat(f_setup.script_name, EXT_SCRIPT);

    f_setup.command_name = (char *)malloc((strlen(f_setup.story_name) + strlen(EXT_COMMAND)) * sizeof(char) + 1);
    strcpy(f_setup.command_name, f_setup.story_name);
    strcat(f_setup.command_name, EXT_COMMAND);
}

bool os_process_arguments(int argc, char *argv[])
{
    int c;
	...
	case 'R': f_setup.restricted_path = strdup(zoptarg); break;
	...
	os_init_startup_variables(argv[zoptind]);
