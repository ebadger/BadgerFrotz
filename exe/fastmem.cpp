...
void init_memory (void)
{
    ...
    for (size = 64; size < story_size; size += n) 
	{
		...
		if (fread (pcp, 1, n, story_fp) != n)
			os_fatal ("Story file read error");
	#endif
		memcpy(pcp, &story_data[story_pos], n);
		story_pos += n;
    }
...
#else
		story_pos = 0;
		memcpy(zmp, &story_data[story_pos], h_dynamic_size);
		story_pos += h_dynamic_size;
#endif
    } 
	else 
	{
...
static void get_default_name (char *default_name, zword addr)
{
	...
	{
		strcpy (default_name, f_setup.aux_name);
	}
