...
static void dumb_getline(char *s)
{
    int c;
    char *p = s;
    memset(p, 0, sizeof(char) * INPUT_BUFFER_SIZE);
...
static void translate_special_chars(char *s)
{
  ...
  while (*src)
    ...
    case '\\':
      ...
      case '\b': *dest++ = ZC_BACKSPACE; break;
    ...
    else if (!strcmp(command, "help")) 
    {
      ...
      else 
      {
        ...
        for (;;) 
        {
          ...
          print_string("HELP: Type <return> for more, or q <return> to stop: ");
          dumb_getline(s);
...
{
  dumb_read_line(s, (char *)prompt, 0, 0, (input_type)0, 0);
