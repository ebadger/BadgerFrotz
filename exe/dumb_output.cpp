...
void os_set_cursor(int row, int col)
{
	...
	setCaretPos(cursor_col, cursor_row);
}
			   ...
			   int src_row, int src_col)
{
	...
	os_SetCell(dest_col, dest_row, dumb_row(src_row)[src_col]);
...
void os_backspace()
{
	if(cursor_col > 0)
	{		
		os_set_cursor(cursor_row + 1, cursor_col);
	}

	dumb_set_cell(cursor_row, cursor_col, make_cell(current_style, ' '));
...
void dumb_display_user_input(char *s)
{
    ...
    while (*s)
	{
		os_SetCell(cursor_row, cursor_col, *s);
...
void dumb_discard_old_input(int num_chars)
{
   ...
   * Erase them.  */
	...
	{
		cursor_col = 0;
		setCaretPos(cursor_row, cursor_col);
	}
...
static void show_cell(cell cel)
{
    ...
    switch (cell_style(cel)) 
	{
		case 0:
			...
			break;
		case PICTURE_STYLE:
			//PrintChar(show_pictures ? c : ' ', false);
			break;
    	case REVERSE_STYLE:
			if (c == ' ')
			{
				//PrintChar(rv_blank_char, true);
			}
			else
			{
				switch (rv_mode) 
				{
					case RV_NONE: 
						...
						break;
					case RV_CAPS: 
						//PrintChar(toupper(c), true);
						break;
					case RV_UNDERLINE: 
						//putchar('_'); 
						//putchar('\b'); 
						//PrintChar(c, true); // TODO: underline 
						break;
					case RV_DOUBLESTRIKE: 
						//putchar(c); 
						//putchar('\b'); 
						//putchar(c); 
						//PrintChar(c, true); // TODO: doublestrike
						break;
				}
			}
			break;
...
static void show_line_prefix(int row, char c)
{
	...
	{
		char buf[255];
		sprintf(buf, (row == -1) ? ".." : "%02d", (row + 1) % 100);
		print_string(buf);
	}
...
void dumb_show_screen(bool show_cursor)
{
	...
	for (int r = hide_lines; r < h_screen_rows; r++)
	{
		show_row(r);
	}
...
#if 0
		const char *p = "sample reverse text";
		for (; *p; p++)
		{
